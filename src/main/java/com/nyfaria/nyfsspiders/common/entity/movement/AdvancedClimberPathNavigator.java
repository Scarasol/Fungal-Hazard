...
import com.nyfaria.nyfsspiders.common.entity.mob.ClimberMoveController;
    ...
    public void tick() {
        ...
        if (!this.isDone()) {
            ...
            if (!this.isDone()) {
                ...
                if (moveController instanceof ClimberMoveController climberMoveController && targetPoint instanceof DirectionalPathPoint directionalPathPoint) {
                    Direction pathSide = directionalPathPoint.getPathSide();
                    if (pathSide != null) {
                        climberMoveController.setMoveTo(targetPos.x, targetPos.y, targetPos.z, targetPoint.asBlockPos().relative(dir), pathSide, this.speedModifier);
                        return;
                    }
                }
    ...
    protected void followThePath() {
        ...
        for (int i = 4; i >= 0; i--) {
            if (this.path.getNextNodeIndex() + i < this.path.getNodeCount()) {
                ...
                } else if (currentTarget instanceof DirectionalPathPoint directionalPathPoint) {
                    Direction targetSide = directionalPathPoint.getPathSide();
        ...
        if (this.findDirectPathPoints) {
            ...
            for (int i = firstDifferentHeightPoint - 1; i >= this.path.getNextNodeIndex(); --i) {
                if (this.canMoveDirectly(pos, this.path.getEntityPosAtNode(this.mob, i))) {
            ...
            if (!pos.closerThan(currentTarget, 2.0D)) {
                ...
                if (targetDir.dot(currentDir) > 0.0D) {
                    ...
                    switch (this.verticalFacing.getAxis()) {
                        ...
                        default -> throw new IllegalStateException("Unexpected axis: " + this.verticalFacing.getAxis());
                    }
            ...
            for (int obx = bx; obx < bx + sizeX2; ++obx) {
                for (int obz = bz; obz < bz + sizeZ2; ++obz) {
                    ...
                    if (offsetX * dx + offsetZ * dz >= minDotProduct) {
                        ...
                        if (nodeTypeBelow == BlockPathTypes.WATER || nodeTypeBelow == BlockPathTypes.LAVA || nodeTypeBelow == BlockPathTypes.OPEN) {
                            return false;
                        }
